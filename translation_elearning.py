...
input_path = fd.askopenfilenames(title='Sélectionnez un fichier', filetypes=[("Sous-titres","*.srt; .vtt")])
...
for file in input_path:
    ...
    # Store the original extension of the file
    original_ext = os.path.splitext(file)[1]

    # Change the extension of the file, because DeepL won't translate files with a .vtt or .srt extension
    ...
    # Rename files even when an error occurs
    finally:
        os.rename(new_file_name, file[:-4] + original_ext)
    
    # Check if the original file name ends with a language code
    if file_name[-6:-4].isalpha():

        # Get the original language code
        original_lang_code = file_name[-6:-4]

        # Set the output path for the translated files, if it's American English (the usual choice) just put "EN" instead of "EN-US", like the other files
        if target_lang == "EN-US":
            os.rename(output_file, output_file[:-6] + "EN" + original_ext)
        else:
            os.rename(output_file, output_file[:-6] + target_lang + original_ext)
